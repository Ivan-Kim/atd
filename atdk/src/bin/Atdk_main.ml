(*
   Entry point to the atdk command.
*)
...
let run conf =
  if conf.version then (
    print_endline Atdk.Version.version;
    exit 0
  )
  else
    conf.input_files
    |> List.iter (fun atd_file ->
      Atdk.Codegen.run_file atd_file
    )
...
let version_term =
  let info =
    Arg.info ["version"]
      ~doc:"Prints the version of atdk and exits"
  in
  Arg.value (Arg.flag info)

let doc =
  "Type-safe JSON serializers for Kotlin"
...
let man = [
  ...
  `P "atdk turns a file containing type definitions into Kotlin classes \
  ...
  `P "The following is a sample ATD file. 'sample.atd' becomes 'sample.kt' \
      with the command 'atdk sample.atd'.";
  `Pre "\
...
type foo = {
  ...
  ~price <kotlin default=\"0.99\">: float;  (* explicit default *)
  items: bar list;
}
