(*
   Entry point to the atdswift command.
*)
...
let run conf =
  if conf.version then (
    print_endline Atdswift.Version.version;
    exit 0
  )
  else
    conf.input_files
    |> List.iter (fun atd_file ->
      Atdswift.Codegen.run_file atd_file
    )
...
let version_term =
  let info =
    Arg.info ["version"]
      ~doc:"Prints the version of atdswift and exits"
  in
  Arg.value (Arg.flag info)

let doc =
  "Type-safe JSON serializers for Swift"
...
let man = [
  ...
  `P "atdswift turns a file containing type definitions into Swift structs \
      that read, write, and validate JSON data. The generated code \
      can be type-checked statically to ensure user code agrees \
  ...
  `P "The following is a sample ATD file. 'sample.atd' becomes 'sample.swift' \
      with the command 'atdswift sample.atd'.";
  `Pre "\
...
type foo = {
  ...
  ~price <swift default=\"0.99\">: float;  (* explicit default *)
  items: bar list;
}
